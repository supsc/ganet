...
func New(strLevel string, pathhead string, pathname string, flag int) (*Logger, error) {
	...
	if pathname != "" {
		...
		s, err := os.Stat(pathname + pathhead)
		if err != nil || !s.IsDir(){
			os.MkdirAll(pathname + pathhead, os.ModePerm)
		}

		file, err := os.OpenFile(path.Join(pathname + pathhead, filename), os.O_RDWR|os.O_CREATE|os.O_APPEND, 0666) //Create(path.Join(pathname, filename))
		if err != nil {
			return nil, err
		}
